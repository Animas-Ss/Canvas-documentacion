...
/*    //prueba de lo que pasa si hacemos una condicion con el index que hacemos  
      if(this.index % 100 === 0){
      this.radius = Math.floor(Math.random() * 10 + 40);
    } */
    this.x =
    ...
    this.friction = 0.6;// minetras aumenta mas fasil de empujar las particulas es!! base es 0.95
  }
  draw(context) {
    //context.fillStyle='red';// lo sacamos de la funcion de dibujo por cuestiones de rendimiento
    //context.fillStyle = 'hsl(' + this.x * 0.5 + ', 100%, 50%)';
    //TODO: creamos una condicion para generar un nuevo elemento segun una condicion 
    //TODO esta parte genera las lineas segun mientras mas bajemos el divisor mas lineas se generan , ms grande menos lineas ya que menos particulas cumplen el requicito
    if(this.index % 0.5 === 0){
      context.save();
      context.globalAlpha = 0.1;// esto nos genera la visibilidad de nuestras lineas 
      context.beginPath();
...
class Effect {
  constructor(canvas, context) {
    ...
    this.mouse = {
      //TODO para centrar el puntero multiplicamos el total de la ventana por 0.5
      x: this.width *  0.5,
      y: this.height * 0.5,
      pressed: false,
  ...
  createParticles() {
    //TODO: creamos la cantidad de aprticulas que queremos y le pasamos como argumento todo el objeto efecto
    for (let i = 0; i < this.numberOfParticles; i++) {
      //TODO: pasamos tambien el indice para generar tamben otro dibujo
